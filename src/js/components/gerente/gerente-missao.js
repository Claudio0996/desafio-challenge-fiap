...
function createBarChart() {
  ...
  const maxValue =
    Math.ceil(Math.max(...chartData.map((d) => d.completed)) / 7) * 7 + 7;
  ...
  chartData.forEach((data, index) => {
    ...
    const completedBar = document.createElementNS(
    ...
    completedBar.addEventListener("mouseenter", (e) =>
      showTooltip(e, `${data.name}: ${data.completed} concluídas`)
    );
